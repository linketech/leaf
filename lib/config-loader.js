...
const leafConfigFields = ['name', 'description', 'server', 'domain', 'static', 'env', 'build', 'serverless', 'timer', 'vpc', 'timerOnly', 'runtime']
...
async function getConfig(dir, checkSrcPath = true) {
	...
	packageJson4Server.dependencies = Object.assign(
		packageJson4Server.dependencies || {},
	...
	// timer only
	const hasKoaOrExpress = !!(
		config.packageJson.dependencies.koa || config.packageJson.dependencies.express
	)
	if (!hasKoaOrExpress) {
		if (R.isEmpty(config.timer)) {
			throw new Error('No http or timer trigger is declared.')
		} else {
			config.timerOnly = true
		}
	}
